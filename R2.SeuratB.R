...
nbt.data = read.table("FPKM_B.txt", sep="\t", header=T, row.names=1)
...
png("Seurat/B_ViolinPlot.Cell.png",width=1500, height=800)
vlnPlot(nbt,c("GCG","MIR1244-1","INS","SST","RPL7","PRSS1","FOXP1","LOX","CD99"))
#vlnPlot(nbt,c("MIR1244-1","INS","SST","TRY6","LOX","CD99"))
dev.off()
png("Seurat/B_ViolinPlot2.Cell.png",width=1500, height=800)
vlnPlot(nbt,c("GCG","NPY","PRSS1","PRSS2","TRY6","ZEB2"))
dev.off()
png("Seurat/B_ViolinPlot3.Cell.png",width=1500, height=800)
vlnPlot(nbt,c("INS","GCK","GCG","GHRL","NPY","PPY","SST","CD44"))
dev.off()
P.gene <- read.table("P.gene.txt")


png("Seurat/B_Disperse.png",width=800, height=800)
nbt=mean.var.plot(nbt,y.cutoff = 2,x.low.cutoff = 2,fxn.x = expMean,fxn.y = logVarDivMean)
dev.off()


png("Seurat/B_PCA.png",width=800, height=800)
nbt=pca(nbt,do.print=FALSE)
pca.plot(nbt,1,2,pt.size = 2)
dev.off()
png("Seurat/B_VizPCA1.png",width=1200, height=800)
viz.pca(nbt,1:2,font.size=1.2 )
dev.off()

nbt=run_tsne(nbt,dims.use = 1:11,max_iter=2000)
png("Seurat/B_tSNE.png",width=800, height=800)
tsne.plot(nbt,pt.size = 1.5)
dev.off()
nbt=DBclust_dimension(nbt,1,2,reduction.use = "tsne",G.use = 8,set.ident = TRUE)
png("Seurat/B_tSNE.cluster.png",width=800, height=800)
tsne.plot(nbt,pt.size = 1)
dev.off()
png("Seurat/B_tSNE.treecluster.png",width=800, height=800)
nbt=buildClusterTree(nbt,do.reorder = TRUE,reorder.numeric = TRUE,pcs.use = 1:11)
dev.off()

png("Seurat/B_tSNE.labelcluster.png",width=800, height=800)
...
write.table(markers.use,"Seurat/B_Cluster_Gene.xls",row.names=T, col.names=T, sep="\t",quote=FALSE)
#markers.use=markers.all$gene
png("Seurat/B_HeatmapCluster.png",width=800, height=4000)
doHeatMap(nbt,genes.use = markers.use$gene,slim.col.label = TRUE,remove.key = TRUE,cexRow=1)
dev.off()

write.table(markers.all[markers.use$gene,],"Seurat/B_Cluster_Gene.xls",row.names=T, col.names=T, sep="\t",quote=FALSE)
