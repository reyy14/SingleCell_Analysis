...
T2D_Old = HSKJ[,pData(HSKJ)[,1]=="T2D"] #96  
T1D = HSKJ[,pData(HSKJ)[,1]=="T1D"] #96  
T2D_OldC = HSKJ[,pData(HSKJ)[,1]=="T2DC"] #54  
...
for (i in 1:6) {
	...
	if (i==3) {
		My <- Old
		sig_mT <- log(exprs(Old[rownames(P),])+1)
	...
	if (i==4) {
		My <- T2D_Old
		sig_mT <- log(exprs(T2D_Old[rownames(P),])+1)
		nOldA <- length(grep("T2DOld_B", colnames(My)))
		nOldB <- length(grep("T2DOld_C", colnames(My)))
		title = "T2DOld"
	}
	if (i==5) {
		My <- T1D
		sig_mT <- log(exprs(T1D[rownames(P),])+1)
		nOldA <- length(grep("T1D", colnames(My)))
		title = "T1D"
	}
	if (i==6) {
		My <- T2D_OldC
	...
	if (i==1) {
		My <- cbind(Young,Old,T2D_Old, T2D_OldC, T1D)
	...
	remove_gene <- paste("Destiny_Won/Panc/",title,"*",sep="")
	N.cluster=5	
	...
	for (j in 1:N.cluster) {
		...
		clustername = paste("Destiny_Won/Panc/",title,".",colorL[j],j,".txt",sep="")
	    print(clustername)
	...
	if (i==1) {
		...
		for (j in 1:C.cluster) {
			...
			clustername = paste("Destiny_Won/Panc/Column",title,".",colorL[j],j,".txt",sep="")
		    print(clustername)
		    write.table(names(sub.tf[sub.tf==TRUE]),clustername,quote=F,col.names=F,row.names=F)
	...
	pngname = paste("Destiny_Won/Panc/Panc",title,"png",sep=".")
