...
nbt.data = read.table("FPKM_A.txt", sep="\t", header=T, row.names=1)
...
write.table(markers.use,"Seurat/A_Cluster_Gene.xls",row.names=T, col.names=T, sep="\t",quote=FALSE)
#markers.use=markers.all$gene
png("Seurat/A_HeatmapCluster.png",width=800, height=4000)
doHeatMap(nbt,genes.use = markers.use$gene,slim.col.label = TRUE,remove.key = TRUE,cexRow=1)
dev.off()

write.table(markers.all[markers.use$gene,],"Seurat/A_Cluster_Gene.xls",row.names=T, col.names=T, sep="\t",quote=FALSE)
