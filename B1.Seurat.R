...
nbt.data = read.table("/home/skw/Desktop/REy/KKJW-SingleCellT2D_rey/New/FPKM_N.txt", sep="\t", header=T,row.names=1)
...
nbt=setup(nbt,project="NBT",min.cells = 1,names.field = 2,names.delim = "_",min.genes = 1,is.expr=1,)
png("NewS/ViolinPlot.Cell.png",width=1500, height=800)
vlnPlot(nbt,c("GCG","MIR1244-1","INS","SST","RPL7","PRSS1","FOXP1","LOX","CD99"))
#vlnPlot(nbt,c("MIR1244-1","INS","SST","TRY6","LOX","CD99"))
dev.off()
png("NewS/ViolinPlot2.Cell.png",width=1500, height=800)
vlnPlot(nbt,c("GCG","NPY","SNORD83B","ATP5L","SNORD50A","PPIC","TPM1","CEACAM7"))
dev.off()
png("NewS/ViolinPlot3.Cell.png",width=1100, height=400)
vlnPlot(nbt,c("INS","GCG","SST","CD44","PRSS1"),size.use=1)
dev.off()
png("NewS/ViolinPlot5.Cell.png",width=1500, height=800)
vlnPlot(nbt,c("SEC11A","TPD52","NHP2","FIS1","HSPD1","FDPS","TIMM23","FOXP1","ACVR2A"))
dev.off()
#stop("d")
P.gene <- read.table("P.gene.txt")
png("NewS/ViolinPlot4.Cell.png",width=3200, height=3000)
...
png("NewS/GCGtest.png",width=800,height=800)
par(mar=c(10,10,5,5))
...
#library("rgl")
png("NewS/GCGtest3D.png",width=800,height=800)
#for (i in 1:length(unique(nbt@ident))){
#	pA <- t(nbt.data[,nbt@ident==unique(nbt@ident)[i]]["GCG",])
#	pB <- t(nbt.data[,nbt@ident==unique(nbt@ident)[i]]["INS",])
...
png("NewS/Heatmap.png",width=800, height=2000)
...
markers.use=subset(markers.all,avg_diff>0&power>0.8)$gene
#markers.use=markers.all$gene
png("NewS/HeatmapCluster.png",width=800, height=2000)
doHeatMap(nbt,genes.use = markers.use,slim.col.label = TRUE,remove.key = TRUE,cexRow=1)
dev.off()

write.table(markers.all[markers.use,],"NewS/Cluster_Gene.xls",row.names=T, col.names=T, sep="\t",quote=FALSE)
...
png("NewS/testExp.png",width=800, height=800)
vlnPlot(nbt,c("HPGD","FN1","NANOG","TDGF1"))
dev.off()

genes.viz = c("POU5F1","NANOG","KRT7","XIST")
png("NewS/GeneFeature.png",width=800, height=800)
