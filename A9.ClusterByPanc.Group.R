...
for (i in 1:5) {
	...
	if (i==3) {
		...
		nOldA <- length(grep("T2D_B", colnames(My)))
		nOldB <- length(grep("T2D_C", colnames(My)))
		title = "T2DOld"
	}
	if (i==4) {
		My <- T1D
	...
	if (i==5) {
		...
		nOldA <- length(grep("T2CD_", colnames(My)))
		title = "T2DOldC"
	}
	
	if (i<=3) rCol = c(rep(col2hex("khaki"),nOldA), rep(col2hex("magenta"),nOldB))
