...
nbt.data = read.table("/home/skw/Desktop/REy/KKJW-SingleCellT2D_rey/New/FPKM_final.txt", sep="\t", header=T,row.names=1)
...
nbt=setup(nbt,project="NBT",min.cells = 1,names.field = 3,names.delim = "_",min.genes = 1,is.expr=1,)
png("NewS_norm/ViolinPlot.Cell.png",width=1500, height=800)
vlnPlot(nbt,c("GCG","MIR1244-1","INS","SST","RPL7","PRSS1","FOXP1","LOX","CD99"))
#vlnPlot(nbt,c("MIR1244-1","INS","SST","TRY6","LOX","CD99"))
dev.off()
png("NewS_norm/ViolinPlot2.Cell.png",width=1500, height=800)
vlnPlot(nbt,c("GCG","NPY","SNORD83B","ATP5L","SNORD50A","PPIC","TPM1","CEACAM7"))
dev.off()
png("NewS_norm/ViolinPlot3.Cell.png",width=1100, height=400)
vlnPlot(nbt,c("INS","GCG","SST","CD44","PRSS1"),size.use=1)
dev.off()
png("NewS_norm/ViolinPlot5.Cell.png",width=1500, height=800)
vlnPlot(nbt,c("SEC11A","TPD52","NHP2","FIS1","HSPD1","FDPS","TIMM23","FOXP1","ACVR2A"))
dev.off()
#stop("d")
P.gene <- read.table("P.gene.txt")
png("NewS_norm/ViolinPlot4.Cell.png",width=3200, height=3000)
...
png("NewS_norm/GCGtest.png",width=800,height=800)
par(mar=c(10,10,5,5))
...
#library("rgl")
png("NewS_norm/GCGtest3D.png",width=800,height=800)
#for (i in 1:length(unique(nbt@ident))){
#	pA <- t(nbt.data[,nbt@ident==unique(nbt@ident)[i]]["GCG",])
#	pB <- t(nbt.data[,nbt@ident==unique(nbt@ident)[i]]["INS",])
...
#stop("d")
png("NewS_norm/Disperse.png",width=800, height=800)
nbt=mean.var.plot(nbt,y.cutoff = 2,x.low.cutoff = 2,fxn.x = expMean,fxn.y = logVarDivMean)
dev.off()


png("NewS_norm/PCA.png",width=800, height=800)
nbt=pca(nbt,do.print=FALSE)
pca.plot(nbt,1,2,pt.size = 2)
dev.off()
png("NewS_norm/VizPCA1.png",width=1200, height=800)
viz.pca(nbt,1:2,font.size=1.2 )
dev.off()

nbt=run_tsne(nbt,dims.use = 1:11,max_iter=2000)
png("NewS_norm/tSNE.png",width=800, height=800)
tsne.plot(nbt,pt.size = 1.5)
dev.off()
nbt=DBclust_dimension(nbt,1,2,reduction.use = "tsne",G.use = 10,set.ident = TRUE)
png("NewS_norm/tSNE.cluster.png",width=800, height=800)
tsne.plot(nbt,pt.size = 1)
dev.off()
png("NewS_norm/tSNE.treecluster.png",width=800, height=800)
nbt=buildClusterTree(nbt,do.reorder = TRUE,reorder.numeric = TRUE,pcs.use = 1:11)
dev.off()

png("NewS_norm/tSNE.labelcluster.png",width=800, height=800)
tsne.plot(nbt,do.label=TRUE, label.pt.size=1, label.cex.text = 2)
dev.off()

for (i in 1:max(nbt@ident)){
	table = names(nbt@ident[nbt@ident==i])
	fname = paste("NewS_norm/Cluster.",as.character(i),".txt",sep="")
	write.table(table,file=fname,sep="\t",quote=F,row.names=F,col.names=F)
	print(fname)
}

save(nbt,file="seurat_files/A2.Robja")
...
png("NewS_norm/Heatmap.png",width=800, height=2000)
...
markers.use=subset(markers.all,avg_diff>0&power>0.8)$gene
#markers.use=markers.all$gene
png("NewS_norm/HeatmapCluster.png",width=800, height=2000)
doHeatMap(nbt,genes.use = markers.use,slim.col.label = TRUE,remove.key = TRUE,cexRow=1)
dev.off()

write.table(markers.all[markers.use,],"NewS_norm/Cluster_Gene.xls",row.names=T, col.names=T, sep="\t",quote=FALSE)
...
png("NewS_norm/testExp.png",width=800, height=800)
vlnPlot(nbt,c("HPGD","FN1","NANOG","TDGF1"))
dev.off()

genes.viz = c("POU5F1","NANOG","KRT7","XIST")
png("NewS_norm/GeneFeature.png",width=800, height=800)
