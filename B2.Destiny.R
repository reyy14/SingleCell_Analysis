...
set.seed(1)
fpkm <- read.table("/home/skw/Desktop/REy/KKJW-SingleCellT2D_rey/New/FPKM_cpms-removed.txt",header=T)
...
#fpkm<-fpkm[!d2,]
print(dim(fpkm))
write.table(fpkm,"Destiny_norm/Exp.txt",col.names=T, row.names=T, sep="\t", quote=F)
...
write.table(fpkm2,"Destiny_norm/ExpExt.txt",col.names=T, row.names=T, sep="\t", quote=F)
colO <- grep("X",colnames(fpkm2))
fpkm3 <- fpkm2[,c(colO)]
write.table(fpkm3,"Destiny_norm/FPKM_X.txt",col.names=T, row.names=T, sep="\t", quote=F)
...
write.table(fpkm2,"Destiny_norm/FPKM_OY.txt",col.names=T, row.names=T, sep="\t", quote=F)
